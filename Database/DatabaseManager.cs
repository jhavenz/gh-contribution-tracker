...
namespace GitHubIssueTracker.Database
{
    ...
    public class DatabaseManager
    {
        ...
        // Removed SaveUser private helper method
        ...
        public void SaveIssues(List<GitHubIssue> issues)
        {
            ...
            foreach (var issue in issues)
            {
                ...
                var existingIssue = context.Issues.Local.FirstOrDefault(i => i.Id == issue.Id) ?? context.Issues.FirstOrDefault(i => i.Id == issue.Id);
                if (existingIssue != null)
                {
                    // Update existing issue - EF Core tracks changes automatically if attached
                    context.Entry(existingIssue).CurrentValues.SetValues(issue);
                    // Manually update collections if necessary (EF Core might handle this depending on config)
        ...
        public List<ContributionReport> GenerateContributionReport(string? developerLogin = null, DateTime? startDate = null, DateTime? endDate = null)
        {
            ...
            var contributions = query
                ...
                .Where(x => string.IsNullOrEmpty(developerLogin) || x.AssigneeLogin == developerLogin) // Ensure we only get relevant assignee's data
                .Select(x => new
                {
                    x.AssigneeLogin,
                    Detail = new ContributionDetail
                    {
                        ...
                        // CreatedAt = x.Issue.CreatedAt, // Removed incorrect mapping
